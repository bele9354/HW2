...
function begin_play(){
	...
	if((player1.value == "") || (player2.value == "")) {
		return alert("Please add two players");
	} else 
		if(!game_started()) {
			started = true;
			player1.disabled = true;
			player2.disabled = true;
			player1.value += " (shape: X)";
			player2.value += " (shape: O)";
			document.getElementById("turn_info").innerHTML = "Turn: Player 1";
		}
...
function clearBoard() {
	...
	for (i = 0; i < ids.length; i++) {
		var square = document.getElementById(table_ids[i]);
		square.innerHTML = ids[i];
	}
}
...
function play() {
	...
	if(!started) {
		return alert("Game has not started yet");
	}

	if (table_ids.includes(input)){ // valid selection
		var selectedSquare = document.getElementById(input);
		var selectedIndex = table_ids.indexOf(input);
	
		if (board_state[selectedIndex] == -1) {
			if (turn == 1) { // player1 turn
				selectedSquare.innerHTML = "X";
				document.getElementById("turn_info").innerHTML = "Turn: Player 2";
				board_state[selectedIndex] = 1;
				toggle_move();
			} else {
				selectedSquare.innerHTML = "O";
				document.getElementById("turn_info").innerHTML = "Turn: Player 1";
				board_state[selectedIndex] = 0;
				toggle_move();
			}
			checkIfWinner();
		} else {
			return alert("That choice is unavailable. Please pick a different square.");
		}
	} else {
		return alert("That choice is unavailable.");
	}
}

function checkIfWinner() {

	for (i = 0; i<=6; i += 3) { // horizontal win
		if ((board_state[i] == 1) && (board_state[i+1] == 1) && (board_state[i+2] == 1)) {
			displayWin("Player 1");
		} else if ((board_state[i] == 0) && (board_state[i+1] == 0) && (board_state[i+2] == 0)) {
			displayWin("Player 2");
		}
	}
	...
	for (i = 0; i<3; i++) { // vertical win
		if ((board_state[i] == 1) && (board_state[i+3] == 1) && (board_state[i+6] == 1)) {
			displayWin("Player 1");
		} else if ((board_state[i] == 0) && (board_state[i+3] == 0) && (board_state[i+6] == 0)) {
			displayWin("Player 2");
		}
	}
	...
	if ((board_state[0] == 1) && (board_state[4] == 1) && (board_state[8] == 1)) {
		displayWin("Player 1");
	} else if ((board_state[i] == 0) && (board_state[4] == 0) && (board_state[8] == 0)) {
		displayWin("Player 2");
	}
